...
/*
	...
	In fact, this is a very convenient desingularization, where things become clear!
	The points at infinity (z==0) of this model are in (x:y:t:z) coordinates:
	(0:sqrt(d):1:0), (0:-sqrt(d):1:0), (sqrt(d/a):0:1:0), (-sqrt(d/a):0:1:0)
	...
	Theorem:
	for P,Q rational, the extended Ewards addition law given above for P1 + P2 does not work if and only if P1 - P2 is a (rational, order-2) point at infinity.

	Consequences:
	The addition law works for all points in the subgroup of size 2*p253, generated by the large-prime p253 subgroup and the non-infinite point of order 2.
...
// These are the three points of order 2 that we can represent with extenden twisted coordinates. None of these is in the p253-subgroup, of course.
var (
	orderTwoPoint      Point_xtw = Point_xtw{x: FieldElementZero, y: FieldElementMinusOne, t: FieldElementZero, z: FieldElementOne}
...
func (P *Point_xtw) IsZero() bool {
	// NOTE: This asserts that P is in the correct subgroup. Otherwise the affine order-2 point (x=0, y=-c, t=0, z=c) would also return true
	return P.x.IsZero()
}
...
func (out *Point_xtw) add_xxx(input1, input2 *Point_xtw) {
	...
	if !out.verify_Point_on_Curve() {
		...
		out.t.MulEq(&E)
		out.z.MulEq(&E)
		fmt.Print("Point not on curve encountered. Point is x==", out.x.String(), " y== ", out.y.String(), "t== ", out.t.String(), " z== ", out.z.String(), "\n")
	}
}
...
// is_equal_safe_xx checks whether p1 == p2. This works for all rational points (including points at infinity), not only those in the subgroup.
...
func (output *Point_xtw) psi_exceptional_xx(input *Point_xtw) {
	var t2, x2, tz, bxx, A, B, C FieldElement

	t2.Square(&input.t)
	x2.Square(&input.x)
	tz.Mul(&input.t, &input.z)
	bxx.Mul(&x2, &endo_b_fe)
	A.Mul(&t2, &endo_a1_fe)
	C.Mul(&x2, &endo_a2_fe) // C acts a temporary, will be overwritten
	A.AddEq(&C)             // A == a1*t^2 + a2*x^2

	B.Mul(&t2, &endo_b_fe)
	B.SubEq(&bxx)
	B.SubEq(&bxx)
	B.AddEq(&x2) // B = b*t^2 + x^2 - 2bx^2

	C.Sub(&t2, &bxx) // C = t^2 - bx^2

	fmt.Println(A.String(), B.String(), C.String(), tz.String())
...
func (output *Point_xtw) psi_regular_xx(input *Point_xtw) {
	var y2, z2, yzz, bzz, A, B, C FieldElement

	y2.Square(&input.y)
	z2.Square(&input.z)
	yzz.Mul(&z2, &input.y)
	bzz.Mul(&z2, &endo_b_fe)
	A.Mul(&y2, &endo_a1_fe)
	C.Mul(&z2, &endo_a2_fe) // C acts a temporary, will be overwritten
	A.AddEq(&C)
	A.MulEq(&input.x) // A == x* (a1*y^2 + a2*z^2)

	B.Mul(&y2, &endo_b_fe)
	B.SubEq(&bzz)
	B.SubEq(&bzz)
	B.AddEq(&z2) // B = b*y^2 + z^2 - 2bz^2

	C.Sub(&y2, &bzz) // C = y^2 - bz^2
