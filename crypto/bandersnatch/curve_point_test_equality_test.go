...
func checkfun_recognize_neutral_exact(s TestSample) (bool, string) {
	...
	return guardForInvalidPoints(expected, singular, "exact testing for neutral element did not work.", s.Points[0].IsNeutralElement_exact)
}
...
func checkfun_recognize_equality_exact(s TestSample) (bool, string) {
	...
	return guardForInvalidPoints(expected, singular, "equality testing failed", s.Points[0].IsEqual_exact, s.Points[1])
}
